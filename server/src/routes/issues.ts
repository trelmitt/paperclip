...
export function issueRoutes(
  ...
  // G (per-issue runner override) write-time authority gate. Thin wrapper binding `db`;
  // the shared implementation (services/issue-assignee-adapter-override-gate.ts) is also
  // called on the plugin-host / SDK path so no caller can bypass it.
  async function assertIssueAssigneeAdapterOverrideAllowed(
    ...
    return assertIssueAssigneeAdapterOverrideAllowedShared(db, companyId, input, fallbackAssigneeAgentId);
  }
