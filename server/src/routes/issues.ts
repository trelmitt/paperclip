...
export function issueRoutes(
  ...
  //   2. Model/adapter coherence (Phase 1): the whole run keys off the override
  //      adapter, so the effective model must exist for it — else the run fails
  //      deep at launch instead of loudly at the write.
  //   3. Built-in allowlist: `allowedAdapterTypes` exists ONLY on built-in agent
  ...
  async function assertIssueAssigneeAdapterOverrideAllowed(
    ...
    // Resolve the effective assignee once; both the model check (inherited
    // model) and the built-in allowlist need its configured adapter.
    ...
    // Check 2 — model/adapter coherence. A modelProfile-only override resolves
    // per-adapter at run time (coherent by construction) and is skipped;
    // otherwise the literal model — the override's own, or the agent's inherited
    // one — must exist for the target adapter. Adapters that list no models
    // (process/dynamic) soft-skip.
